...
void ZoneTree::buildDefault() {
	insert(5, "Forest-Root", 0.0f);
	insert(3, "Zone-A", 0.3f);
	insert(7, "Zone-B", 0.4f);
	insert(1, "Zone-A1", 0.2f);
	insert(4, "Zone-A2", 0.5f);
	insert(6, "Zone-B1", 0.3f);
	insert(9, "Zone-B2", 0.6f);
...
float TerrainTree::computeTerrainRisk(float slope, float dry, float density) {
	float risk = (slope + dry + density) / 3.0f;
	cout << "Terrain Risk = (" << slope << " + " << dry << " + " << density << ") / 3 = " << risk << endl;
	if (risk > 0.7f) {
		cout << "HIGH terrain risk." << endl;
	}
	else if (risk > 0.4f) {
...
void TerrainTree::buildDefault() {
	insert(5, "Terrain-Root", 0.5f);
	insert(2, "Low-Risk-Plains", 0.2f);
	insert(8, "High-Risk-Hills", 0.8f);
	insert(1, "Safe-Valley", 0.1f);
	insert(3, "Dry-Brush", 0.6f);
	insert(7, "Rocky-Slope", 0.7f);
	insert(9, "Dense-Forest", 0.9f);
...
void ResourceTree::buildDefault() {
	insert(5, "Resource-Root", 0.5f);
	insert(3, "Water-Tank-A", 0.8f);
	insert(7, "Fire-Truck-B", 0.9f);
	insert(1, "Pump-Station-1", 0.6f);
	insert(4, "Hose-Unit-2", 0.7f);
	insert(6, "Helicopter-1", 0.95f);
	insert(9, "Ground-Team-A", 0.5f);
...
float IncidentTree::computeFireLevel(float alpha, float temp, float beta, float smoke) {
	...
	if (level > 0.7f) {
		cout << "MAJOR fire alert." << endl;
	}
	else if (level > 0.4f) {
		cout << "MODERATE fire risk." << endl;
	}
	else {
...
void IncidentTree::buildDefault() {
	insert(5, "Incident-Root", 0.5f);
	insert(3, "Fire-Zone-3", 0.8f);
	insert(7, "Wildlife-Zone-7", 0.4f);
	insert(1, "Human-Activity-1", 0.6f);
	insert(4, "Smoke-Alert-4", 0.7f);
...
void DecisionTree::globalDecision(float totalRisk, float threshold) {
	...
	if (totalRisk > threshold) {
		...
		cout << "STATUS: Global risk within acceptable limits." << endl;
	}
}
...
void DecisionTree::buildDefault() {
	insert(6, "Decision-Root", 0.5f);
	insert(3, "Local-Zone-3", 0.7f);
	insert(9, "Regional-Zone-9", 0.6f);
	insert(1, "Zone-A-Local", 0.4f);
	insert(5, "Zone-B-Regional", 0.5f);
	insert(7, "Global-Alert", 0.9f);
	insert(11, "Emergency-Response", 0.95f);
